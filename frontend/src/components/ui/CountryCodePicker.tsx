...
export const CountryCodePicker = ({
  ...
  const selectedCountry = COUNTRIES.find((c) => c.dialCode === value) || COUNTRIES[0]; // Default to UK
    ...
    <div>
      ...
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="w-full border rounded-lg p-1 flex items-center justify-between bg-white hover:bg-muted/50 transition-colors"
        >
          <span className="flex items-center ml-2 gap-2">
            <span className="text-lg">{selectedCountry.flag}</span>
            <span className="text-md">+{selectedCountry.dialCode}</span>
          </span>
              ...
              {filteredCountries.length > 0 ? (
                filteredCountries.map((country) => (
                  <button
                    ...
                    className={`w-full p-2.5 text-left hover:bg-muted/50 transition-colors flex items-center justify-between text-sm ${
                      country.dialCode === value ? 'bg-primary/5' : ''
                    }`}
                  >
                    ...
                    {country.dialCode === value && <span className="text-primary font-bold">✓</span>}
                  </button>
                ))
              ) : (
                <div className="p-2.5 text-sm text-muted-foreground text-center">No countries found</div>
              )}
            </div>
          </div>
        )}
